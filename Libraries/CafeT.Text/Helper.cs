...
namespace CafeT.Text
{
    public static class StringHelper
    {
        ...
        public static char[] delimiters_sentence = new char[] {
            '.','?','!'
        };
        ...
        /// <summary>
        /// Print list of string to text [0-->n]
        /// </summary>
        /// <param name="words"></param>
        /// <returns></returns>
        public static string ToText(this List<string> words)
        {
        ...
        /// <param name="text"></param>
        /// <returns></returns>
        public static string[] ExtractSentences(this string text)
        {
            if (!text.IsNullOrEmptyOrWhiteSpace())
            {
                text = text.ToStandard();
                string[] sentences = text.Split(delimiters_sentence, StringSplitOptions.RemoveEmptyEntries);
                List<string> results = new List<string>();
        ...
        /// <summary>
        /// return all word with all index of word
        /// By: Phan Minh Tài
        /// </summary>
        /// <param name="text"></param>
        /// <returns></returns>
        public static Dictionary<string, int[]> ToWordsWithIndex(this string text)
        {
            Dictionary<string, int[]> dict = new Dictionary<string, int[]>();
            string[] words = text.ToWords().Distinct().ToArray();
            foreach (string word in words)
            {
                dict.Add(word, text.IndexAll(word));
            }
            return dict;
        }

        public static string InsertWord(this string text, string word, int index)
        {
            string retsult = string.Empty;
            string[] words = text.ToWords();
            ...
            {
                for (int i = words.Length; i < index; i++)
                {
                    newWords.Add("_");
                    if (i == index - 1)
                    {
            ...
            retsult = newWords.ToAsString();
            return retsult;
        }
        public static string ToAsString(this List<string> list)
        {
            string result = string.Empty;
        ...
        //public static string DictionaryToText(this Dictionary<string,int[]> dict)
        //{
        ...
        public static bool StartWithLower(this string text)
        {
            if (!text.IsNullOrEmptyOrWhiteSpace())
            {
                if (char.IsLower(text.ToCharArray()[0])) return true;
            }
            return false;
        }
        ...
        public static string DeleteBeginTo(this string text, string to)
        {
            ...
            return text.Substring(_firstIndex + 1);
        }
